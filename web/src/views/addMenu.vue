<template>
    <el-form style="width: 40%" :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="父类ID" prop="pid">
            <el-select v-model="ruleForm.pid" placeholder="请选择父类">
      ...
      return {
        ...
        rules: {
          ...
          orderNum: [
            ...
            { min: 0, max: 7, message: '长度在 1 到 7 个字符', trigger: 'blur' }
          ],
          url: [
            { required: true, message: '请输入菜单地址', trigger: 'blur' },
            { min: 1,  message: '最少长度在1个字符', trigger: 'blur' }
          ],
          pid: [
            { required: true, message: '请选择父类ID', trigger: 'change' }
          ],
